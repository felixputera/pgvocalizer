...
def _stringify_pg_dt_fun(string):
    # age(timestamp)
    swap_list = []
    for func in re.finditer(r'age\(([(\']?[0-9a-zA-Z._]+\(?\)?[)\']?)\)', string):
    ...
    swap_list = []
    for func in re.finditer(r'age\(([(\']?[0-9a-zA-Z._]+\(?\)?[)\']?)[ ]*,[ ]*([(\']?[0-9a-zA-Z._]+\(?\)?[)\']?)\)', string):
    ...
    swap_list = []
    for func in re.finditer(r'date_part\(([(\']?[0-9a-zA-Z._]+\(?\)?[)\']?)[ ]*,[ ]*([(\']?[0-9a-zA-Z._]+\(?\)?[)\']?)\)', string):
    ...
    swap_list = []
    for func in re.finditer(r'date_trunc\(([(\']?[0-9a-zA-Z._]+\(?\)?[)\']?)[ ]*,[ ]*([(\']?[0-9a-zA-Z._]+\(?\)?[)\']?)\)', string):
    ...
    swap_list = []
    for func in re.finditer(r'extract\((([(\']?[0-9a-zA-Z._]+\(?\)?[)\']?[ ]*)+)\)', string):
    ...
    swap_list = []
    for func in re.finditer(r'isfinite\(([(\']?[0-9a-zA-Z._]+\(?\)?[)\']?)\)', string):
...
def _stringify_table_column(string):
    swap_list = []
    for table_col in re.finditer(r'([0-9a-zA-Z_\-]+)\.([0-9a-zA-Z_\-]+)', string):
